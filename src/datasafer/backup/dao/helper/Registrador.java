...
public class Registrador {
	...
	public static <T> List<Registro> insere(	Usuario solicitante,
		...
		for (Field atributo : objeto.getClass()
									.getDeclaredFields()) {
			try {
				if (!atributo.isAnnotationPresent(JsonIgnore.class)
						|| (atributo.isAnnotationPresent(JsonProperty.class) && atributo.getAnnotation(JsonProperty.class)
			...
			try {
				if (!atributo.isAnnotationPresent(JsonIgnore.class)
					...
					if (valorOrigem != null && !valorOrigem.equals(valorDestino)) {
						...
						registro.setPara(valorOrigem.getClass() == Date.class ? new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(valorOrigem)
